...
Figure \ref{fig:thermalisation_different_noise_fracs} reports the Langevin evolution of the system for different noise fractions. The system was initialised in the same state for all the configurations on a $32 \times 32$ lattice. The red line corresponds to the case $s=0$, namely a classical simulation, while the blue line corresponds to the case $s=1$, namely the fully quantum case. 
One can notice that as quantum modes are added via coloured noise, the systems shifts its equilibrium point. \\~\\ 
We now want to study observables values at equilibrium, as a function of the noise fraction $s$. We start with a simulation with $s=1$ and we progressively lower the cutoff, keeping fixed all the quantities in the classical action. 
\textcolor{blue}{Each figure reports two plots corresponding to two different parameter configurations.
