...
\begin{equation*}
	...
	\end{aligned}
\end{equation*}
For what concerns the fermionic action, a naive discretisation is not sufficient, due to the well known doubling problem \cite{rothe_LGT,Montvay1994QuantumLattice} \textcolor{blue}{(is it fine if I cite the books? I couldn't really find a paper where the doubling problem was discussed in particular)}. In this work Wilson fermions \cite{wilson_lqcd} are employed as a way to fix such issue. Details of this formulation are explained in Appendix \ref{chap:AppendixB}. Here, only the final discretised action is reported, which reads
\begin{equation}
...
\end{equation}
with $\psi_n$ beeing a two-component spinor, and $\widehat{D}_{m,n}$ beeing the Wilson-Dirac operator \textcolor{blue}{(can I include $g\phi $ in the definition of $D$?)} defined as 
\begin{equation}
    \begin{aligned}
...
\end{equation}
\textcolor{blue}{Both the above and below, can I write $ar$ like that?} \\
...
\begin{equation}
	\begin{aligned}
		S_\text{eff}[\hat\phi] 	&= S_\phi[\hat\phi] - N_f \trover{n,s} \log \widehat{D}_{nn} \\
...
\end{equation}
Stochastic average w.r.t. the measure $P(\eta)$ are computed via 
\begin{equation*}
...
The simplest first-order integration algorithm is \cite{ParisiWu}
\begin{equation*}
...
In this way, for any observable $O$, one can introduce a Monte-Carlo estimator $\expect{O}_*$ which is expected to converges to the expectiation value given by \eqref{eq:expectation_observables} in the limit of infinite samples
\begin{equation}
