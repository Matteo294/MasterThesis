...
\begin{equation*}
	...
	\end{aligned}
\end{equation*}
for $U_L, U_R \in SU(N_f)$. In terms of the full spinor $\psi$, the chiral symmetry can be written as 
\begin{equation*} 
    \psi \to M \, \psi \qquad M = e^{i(\theta_a \tau^a + \gamma_5 \beta_a\tau^a)}, \quad \tau^a \in su(N_f)
\end{equation*} 
so that $SU(N_f) _L\times SU(N_f)_R \simeq SU(N_f)_V \times SU(N_f)_A$, where $SU(N_f)_V$ is the isospin subgroup ($\beta_a =0$) and $SU(N_f)_A$ is the axial rotation subgroup ($\beta_a = 0$). \\
...
In particular, the insertion of the Wilson term in the action, as it will be shown in the next chapter, causes explicit breaking of the symmetry. One cannot solve the problem but can alleviate it \textcolor{red}{cite Ginspar-Wilson, Staggered, \dots}. 
Our approach to study chiral symmetry will be discussed and motivated in section \ref{sec:chiral_PT} and consists in removing the Wilson term, dealing with the doublers.
...
\end{equation}
The action is not invariant under the full chiral group, but if $m_q = 0$ it is symmetric under the discrete chiral group 
\begin{equation*}
    \begin{aligned}
        \phi &\to -\phi, \\
    ...
    \end{aligned}
\end{equation*}
typical of theories such as the Gross-Neveu model \textcolor{red}{citationsssss}. In order to generalise the model to the full chiral group, one has to consider an $O(4)$ scalar sector and interactions, since $O(4) \sim SU(2) \times SU(2)$. Similar effective theories with such properties are, for example, 
the Nambu-Jona-Lasinio model and the Quark-Meson model \textcolor{red}{citationssssss}. Thus, since we are in $1+1$ dimensions, spontaneous breaking of continuous symmetries is forbidden by the Mermin-Wagner theorem \textcolor{red}{citationsssss}. \\~\\
