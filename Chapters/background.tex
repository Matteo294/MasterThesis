...
There are only two exceptions for this, either the correlation length is zero, or infinite. The latter case represents a fixed-point of the RG, and the system exhibits scale invariance. \\
...
\begin{figure}
    ...
    \caption[The Wilsonian renormalisation group]{The Wilsonian perspective of the renormalisation group. Ultraviolet quantum fluctuations above the scale $\Lambda$ are integrated out and effectively encoded in the coarse-grained action $S_{\Lambda}^{\text{eff}}$.}
    \label{fig:wilson}
\end{figure}
\newpage
...
\end{equation}
Expectation values of observables are computed as
\begin{equation}
...
\begin{equation*}
    \beta = 1/T = 1/L_t,
\end{equation*}
and boundary conditions are chosen depending on the spin-statistics of the corresponding particles, namely periodic conditions for bosons, and anti-periodic for fermions
\begin{equation*}
    \begin{aligned}
...
In any case, from this description it should emerge that the spacing $a$ should not be treated as a free parameter in the continuum limit, but rather a dynamically determined quantity that depends on the couplings of the theory.\\~\\
...
For any observable $O$, which is a function of the field, one has, for fixed time $\tau$,
\begin{equation*}
...
the Nambu \textendash{} Jona-Lasinio model \cite{Nambu1961DynamicalI,Nambu1961DynamicalII} and the Quark-Meson model \textcolor{red}{citationssssss}. Thus, since this work is based on $1+1$ spacetime dimensions, spontaneous breaking of continuous symmetries is forbidden by the Mermin-Wagner theorem \cite{merminwagner}, hence we decided to opt for the $O(1)$ version. \\~\\
